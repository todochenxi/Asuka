...
40+ 条已冻结不变量（B/R/S/D/X/A/E/I/L/C/PR/O 系列）都朝这个方向。
反复出现的判据：**宁可拒绝，不许编造**。

## 规模（v2.1.73，2026-09-22）
...
1. **"内存里分得清" ≠ "账本上分得清"**（M79）。查"为什么"先确认字段活在哪层：
   内存字段 / 快照 / 账本，是三件事。
2. ⭐⭐ **承诺句不是机制**（M85）。I-6 docstring 写"只能由这个工厂构造"，
   而 `@dataclass(frozen=True)` 的 `__init__` **是公开的** ——
   **frozen 只保证"不可变"，不保证"构造受控"**。
   ⇒ 判据要住在**所有构造路径的汇合处**（`__post_init__`）；
   补的时候问"除工厂外还有几条路能造出它"（**快照恢复最容易漏**）。
3. ⭐⭐ **断言隔离：除"被测那一项"外，其他字段一律给足**（M85 栽了第二次）。
   断言里 `assertIn("execution_id", str(exc))` 让**拒绝理由点名被测字段**。
   否则多道校验叠加 → 验证的是"某处会拦"而不是"这一处会拦" → 变异假绿。
   与 M82 的"把存在当成被处理"**同一族：断言边界没划在守点上**。
4. **交付的判据是"被用上"，不是"被写出来"**。同一坑栽过五次
   （没人用它启动 / 没人读 stack / 页面没调用 / 实现了没冻结）。
   验收方式 = **改坏它看谁红**。
5. **一个 Store 两种实现（内存 + PG）时，保证必须两边同源**。
   只在一种实现里成立的保证 = 在生产上不成立。
6. **保证住在哪一层，就在哪一层验证**。PG 物理约束必须写 `*_real_pg.py`。
7. **盘点用"有没有目录/文件"代替"能力是否落地"** —— 栽过三次（M4/M7/M14 全判错）。
   正确问法：**这条里程碑描述的那件事，代码里做得到吗？**
8. **并发判胜负唯一可靠写法 = 带条件的 UPDATE + rowcount**，不是"先读再写"。
   S-4 / S-15 / A-11（`AND status='pending'`）、E-25（`AND version=`）、
   PR-3（`AND (claimed_until IS NULL OR <=%s)`）。
9. **空洞登记会过期**，动手前先实证。
10. **变红脚本优先用单行替换**：heredoc 多行替换会把 `\n` 转成 `/n` → 语法错 → 计数 0。
    看到"红了 0 条"先怀疑脚本。
11. **起子进程/开连接的测试，`terminate()` 后必须 `wait()`**，否则残留进程连着 PG
    → 下次 `DROP DATABASE` 被拒 → 偶发红一条无关的灯。
12. ⚠️ **改版本号必须在跑测试之前做完，且不许与跑测试并发**（M84 + M85）。
...
13. **`real_pg()` 默认 `fresh=True` 会重建库**！附加连接必须 `fresh=False`。
14. **断言 API 错误必须比 `.code`，不能比 `str(e)`** —— 错误码在 `.code` 属性上。
15. **改动页面必须跑集成测试**：页面丢了全站 `<h1>` 会让 `test_api_real_http.py` 红。
16. 重启服务要确认日志里有 `Uvicorn running` 再验证（端口被占会测到旧进程）。
17. 偶发红排查：**完整捕获输出写文件再 grep FAIL**，别 `tail -3`。
18. sqlite 替身默认只吃 001；迁移里别写 `COMMENT ON`（PG 方言，sqlite 语法错）。
19. PG 连接要带 `row_factory=dict_row`。
20. ⚠️ **两个 python 不是同一个**：系统 python 没装 `psycopg` / `httpx`。
...
21. **变红脚本要断言"替换命中次数 == 1"** —— 锚点不唯一时静默 SKIP。
    曾栽：`"reason": reason,` 在 loop.py 里出现 4 次。
22. ⭐ **加了字段 ≠ 送达**。判断信息到没到：**沿它实际走的路读到终点那一份**。
23. ⭐⭐ **判据读什么，就决定了它能看见什么**（M81）。I-11 写"数 `execution.failed`
    observation" —— 把"什么算失败"**外包给了写 observation 的人**。
    ⇒ 补完不变量后除了问"还能被绕过吗"，还要问：
    **它依赖的那些事实，是不是每一扇门都在写？**
24. ⭐ **断言"最终不是 X"常常是错的断言**（M81）。写前先问：
    **这条不变量禁止的是"最终状态"还是"直达路径"？**
25. ⭐ **"存在"不等于"被处理"**（M82）。`reducer.py` 末尾有兜底分支，
    任何 observation 都会留在 State 上 → `assertIn(kind, kinds)` 守不住分支。
26. ⭐⭐ **两个现成选项都不对时，缺的是第三个**（M82）。**"不确定"是一种必须被
    记录的状态，不是"没有状态"。**
27. ⭐ **判断不能当结论用**（M83）。"父 Run 自己知道"是**关于系统状态的断言**，
    而系统状态可测。写成有变异守着的测试，它才是一份能承重的结论。
28. ⭐ **不要为不存在的能力写区分**（M83）。**账本上每一列都该对应一条真实路径。**
29. ⭐ **"零代码改动"的一轮是正当的里程碑**（M83）。把"读过代码觉得对"
    变成"有变异守着的结论"，本身就值得单独一轮。
30. ⭐ **诊断也是有归宿的：「写进任务卡」≠「写进基线」**（M84）。
    任务卡是**过程**，基线是**结论**。**这比红灯本身贵得多。**
31. ⭐ **"偶发红"先假定它是真的，但也先翻账本**（M84）。
    **撞不出来 ≠ 问题不存在**，可能只是触发条件不在"跑测试"这个动作里。
32. ⭐ **声明点在哪一层，就在哪一层读它**（M84 写测试时踩的）。
    文档的版本声明在**文件名**上，我却对它 `read_text()` → 永远匹到 0 次。
33. ⚠️ **一致性检查只认"声明点"，不扫全文**（M84）。扫全文会把**历史记录**
    判成**漂移**，造出一条永远红的噪声，**比没有更坏**。
34. ⚠️ 本项目用 **`unittest`**（不是 pytest）。集成命令：
...
- ⚠️ **只改目录名，不要批量替换内容里的 `agentos`**：包名 `packages/agent_*`、
  模块 `apps.*`、文档标题都是内容，批量替换会打红 1299 条测试
